...
- (NSString*)networkTypeDescription {
    switch (_networkType) {
        case RMBTNetworkTypeNone:
            return NSLocalizedStringWithDefaultValue(@"connectivity.not-connected", nil, [NSBundle mainBundle], @"Not connected", @"network type description not connected");
        case RMBTNetworkTypeWiFi:
            return NSLocalizedStringWithDefaultValue(@"connectivity.wifi", nil, [NSBundle mainBundle], @"Wi-Fi", @"network type description wifi");
        case RMBTNetworkTypeCellular:
            if (_cellularCodeDescription) {
                return _cellularCodeDescription;
            } else {
                return NSLocalizedStringWithDefaultValue(@"connectivity.cellular", nil, [NSBundle mainBundle], @"Cellular", @"network type description cellular");
            }
        default:
            NSLog(@"Invalid network type %ld", (long) _networkType); // cast to long to get rid of compiler warning on iphone 4
            return NSLocalizedStringWithDefaultValue(@"intro.network.connection.name-unknown", nil, [NSBundle mainBundle], @"Unknown", @"network type description unknown");
    }
}
...
//                _cellularCodeGenerationString = [self cellularCodeGenerationString:netinfo.currentRadioAccessTechnology];
        ...
        case RMBTNetworkTypeNone:
            break;
        default:
            NSAssert1(false, @"Invalid network type %ld", (long) _networkType);  // cast to long to get rid of compiler warning on iphone 4
    }
}
...
- (NSDictionary*)testResultDictionary {
    ...
    if (code > 0) {
        result[@"network_type"] = [NSNumber numberWithUnsignedLong:code];//[NSNumber numberWithUnsignedInt:code];
    }
