...
- (instancetype)initWithDelegate:(id<RMBTConnectivityTrackerDelegate>)delegate stopOnMixed:(BOOL)stopOnMixed {
    if (self = [super init]) {
        ...
        _delegate = delegate;
        _queue = dispatch_queue_create("com.specure.nettest.connectivitytracker", DISPATCH_QUEUE_SERIAL);
...
- (void)reachabilityDidChangeToStatus:(GCNetworkReachabilityStatus)status {
    ...
    if (networkType == RMBTNetworkTypeNone) {
        NSLog(@"No connectivity detected.");
    ...
    NSLog(@"New connectivity = %@", connectivity.testResultDictionary);
    ...
    if (_stopOnMixed) {
        ...
        if (_lastConnectivity) {
            if (connectivity.networkType != _lastConnectivity.networkType) {
                NSLog(@"Connectivity network mismatched %@ -> %@", _lastConnectivity.networkTypeDescription, connectivity.networkTypeDescription);
                compatible = NO;
            } else if (![connectivity.networkName isEqualToString:_lastConnectivity.networkName]) {
                NSLog(@"Connectivity network name mismatched %@ -> %@", _lastConnectivity.networkName, connectivity.networkName);
                compatible = NO;
            }
        }
