...
class RMBTTestResult : NSObject {
    ...
    func addPingWithServerNanos(serverNanos: UInt64, clientNanos: UInt64) {
        ...
        let sortedPings = pings.sorted { (p1: RMBTPing, p2: RMBTPing) -> Bool in
    ...
    private func updateTotalHistory() -> [RMBTThroughput]! {
        ...
        for var i = maxFrozenPeriodIndex + 1; i <= commonFrozenPeriodIndex; i++ {
            if (i == commonFrozenPeriodIndex && (currentHistories.objectAtIndex(0) as! RMBTThroughputHistory).isFrozen) { //currentHistories[0].isFrozen) {
                ...
                for var threadIndex = 0; threadIndex < threadCount; threadIndex++ {
                    let threadLastTput = currentHistories[threadIndex].periods[Int(minPeriodIndex)]
                    // Factor = (t*-t(k,m-1)/t(k,m)-t(k,m-1))
                    var factor = Double(minEndNanos - threadLastTput.startNanos) / Double(threadLastTput.durationNanos)
        ...
        var result = (totalCurrentHistory.periods as NSArray).subarrayWithRange(NSMakeRange(maxFrozenPeriodIndex + 1, commonFrozenPeriodIndex - maxFrozenPeriodIndex)) as! [RMBTThroughput]
    ...
    func startDownloadWithThreadCount(threadCount: Int) {
        ...
        for var i = 0; i < threadCount; i++ {
            perThreadDownloadHistories./*append*/addObject(RMBTThroughputHistory(resolutionNanos: resolutionNanos))
            perThreadUploadHistories./*append*/addObject(RMBTThroughputHistory(resolutionNanos: resolutionNanos))
        }
    ...
    func flush() -> [AnyObject]! {
        ...
        for h in currentHistories {
            (h as! RMBTThroughputHistory).squashLastPeriods(1 + (h.periods.count - totalPeriodCount))
        }
    ...
    func resultDictionary() -> NSDictionary { // -> computed property
        var pingTestResultArray = pings.map { p in
        ...
        var result: [String:AnyObject] = [
            ...
            "test_num_threads":     NSNumber(unsignedInteger: threadCount),
            "publish_public_data":  RMBTSettings.sharedSettings().publishPublicData
        ]
    ...
    private func subresultForThreadThroughputs(perThreadArray: /*[RMBTThroughputHistory]*/NSArray, withDirectionString directionString: String) -> [[String:AnyObject]] {
        ...
        for var i = 0; i < perThreadArray.count; i++ {
            ...
            for t in h.periods {
                ...
                result.append([
                    ...
                    "thread":   NSNumber(unsignedInteger: i),
    ...
    private func locationsResultDictionary() -> [String:[[String:NSNumber]]] {
        ...
        for l in locations {
            ...
            let t = l.timestamp.timeIntervalSinceDate(testStartDate)
    
            let tsNanos = UInt64(t) * NSEC_PER_SEC
            ...
            result.append([
                ...
                "time_ns":  NSNumber(unsignedLongLong: tsNanos),
