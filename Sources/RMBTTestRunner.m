...
- (void)start {
    dispatch_async(_workerQueue, ^{
        ...
        [[ControlServer sharedControlServer] getTestParamsWithParams:params success:^(id testParams) {
            dispatch_async(_workerQueue, ^{
...
- (void)testWorker:(RMBTTestWorker*)worker didFinishDownlinkPretestWithChunkCount:(NSUInteger)chunks withTime:(uint64_t)serverNanos {
    ASSERT_ON_WORKER_QUEUE();
    NSAssert(_phase == RMBTTestRunnerPhaseInit, @"Invalid state");
    NSAssert(!_dead, @"Invalid state");

    //should it be calculated init time recent time - start test time ?
    [_delegate testRunnerDidFinishInit:serverNanos];

    NSLog(@"Thread %lu: finished download pretest (chunks = %lu)", (unsigned long)worker.index, (unsigned long)chunks);
    if (!_singleThreaded && chunks <= _testParams.pretestMinChunkCountForMultithreading) {
        _singleThreaded = YES;
    }
    if ([self markWorkerAsFinished]) {
        if (_singleThreaded) {
            NSLog(@"Downloaded <= %lu chunks in the pretest, continuing with single thread.", (unsigned long)_testParams.pretestMinChunkCountForMultithreading);
...
- (void)testWorkerDidStop:(RMBTTestWorker *)worker {
    ...
    NSLog(@"Thread %lu: stopped", (unsigned long)worker.index);
...
- (void)testWorker:(RMBTTestWorker*)worker didMeasureLatencyWithServerNanos:(uint64_t)serverNanos clientNanos:(uint64_t)clientNanos {
    ...
    NSLog(@"Thread %lu: pong (server = %" PRIu64 ", client = %" PRIu64 ")", (unsigned long)worker.index, serverNanos, clientNanos);
...
- (uint64_t)testWorker:(RMBTTestWorker *)worker didStartDownlinkTestAtNanos:(uint64_t)nanos {
    ...
    NSLog(@"Thread %lu: started downlink test with delay %" PRIu64, (unsigned long)worker.index, nanos-_downlinkTestStartedAtNanos);
...
- (void)testWorker:(RMBTTestWorker*)worker didFinishUplinkPretestWithChunkCount:(NSUInteger)chunks {
    ...
    NSLog(@"Thread %lu: finished uplink pretest (chunks = %lu)", (unsigned long)worker.index, (unsigned long)chunks);

    if ([self markWorkerAsFinished]) {
        NSLog(@"Uplink pretest finished");
...
- (uint64_t)testWorker:(RMBTTestWorker*)worker didStartUplinkTestAtNanos:(uint64_t)nanos {
    ...
    NSLog(@"Thread %lu: started uplink test with delay %" PRIu64, (unsigned long)worker.index, delay);
...
- (void)submitResult {
    dispatch_async(_workerQueue, ^{
        ...
        [[ControlServer sharedControlServer] submitResult:result success:^(id response) {
            dispatch_async(_workerQueue, ^{
    ...
    if (duration > 0) {
        ...
        dispatch_source_set_event_handler(_timer, ^{
            ...
            if (elapsedNanos > _progressDurationNanos) {
                ...
                // ..then kill the timer
                if (_timer) {
                    dispatch_source_cancel(_timer); // TODO: after swift rewrite of AppDelegate one test got an exception here!
                }
                _timer = nil;
...
- (void)applicationDidSwitchToBackground:(NSNotification*)n {
    NSLog(@"App backgrounded, aborting %@",n);
    dispatch_async(_workerQueue, ^{
