...
typedef NS_ENUM(NSInteger, RMBTTestWorkerState) {
...
- (void)fail {
    if (_state == /*RMBTTestWorkerStateAborted*/RMBTTestWorkerStateFailed) return; // already failed
  ...
  // Pretest
    if (tag == RMBTTestTagRxBanner) {
        ...
        if (RMBTCurrentNanos() - _pretestStartNanos >= (_params.pretestDuration * NSEC_PER_SEC)) {
            ...
            [_delegate testWorker:self didFinishDownlinkPretestWithChunkCount:_pretestChunksCount withTime:RMBTCurrentNanos() -_pretestStartNanos];
        } else {
            // ..no, get more chunks
    ...
    // Latency test
        ...
        // -> PING
        _pingSeq++;
        NSLog(@"Ping packet sent (delta = %" PRIu64 ")", RMBTCurrentNanos() - _pingStartNanos);
    ...
    // Uplink test
        ...
        if (_testUploadLastChunkSent) {
            ...
            if (nanos - _testStartNanos >= (_params.testDuration * NSEC_PER_SEC)) {
                NSLog(@"Sending last chunk in thread %lu", (unsigned long)_index);
        ...
        if ([line hasPrefix:@"TIME"]) {
            ...
            if (_testUploadLastChunkSent && now >= _testUploadMaxWaitReachedClientNanos) {
                NSLog(@"Max wait reached in thread %lu. Finalizing.", (unsigned long)_index);
                [self finalize];
                return;
            }
            ...
            if (_testUploadLastChunkSent && now >= _testUploadEnoughClientNanos && ns >= _testUploadEnoughServerNanos) {
                // We can finalize
                NSLog(@"Thread %lu has read enough upload reports at local=%" PRIu64 " server=%" PRIu64 ". Finalizing...", (unsigned long)_index, now - _testStartNanos, ns);
                [self finalize];
                return;
            }
            ...
            NSLog(@"Thread %lu has read ALL upload reports. Finalizing...", (unsigned long)_index);
            [self finalize];
        } else {
            // INVALID LINE
